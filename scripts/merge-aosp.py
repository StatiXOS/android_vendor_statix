...
def list_aosp_repos():
    ...
    aosp_repos = []
    with open(f'{WORKING_DIR}/.repo/manifests/default.xml') as aosp_manifest:
...
def force_sync():
    ...
    for repo in REPOS_TO_MERGE:
        if os.path.isdir(repo):
            shutil.rmtree(repo)
            subprocess.run(
                ['repo', 'sync', '-c', '--force-sync', '-f', '--no-clone-bundle', '--no-tag', '-j$(nproc --all)', '-q']
            )
        else:
            print(f"Unable to find {repo}")
...
def merge():
    ...
    for repo in REPOS_TO_MERGE:
        ...
        try:
            git.cmd.Git('{0}{1} {2}'.format(BASE_URL, repo_str, BRANCH_STR)).pull()
