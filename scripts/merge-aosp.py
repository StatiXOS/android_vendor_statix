...
def force_sync():
    ...
    for repo in REPOS_TO_MERGE:
        ...
        os.system('git fetch statix --unshallow')


def merge():
    """ Merges the necessary repos and appends all repos that fail """
    for repo in REPOS_TO_MERGE:
        repo_str = repo
        os.chdir("{0}/{1}".format(WORKING_DIR, repo))
        if repo == "build/make":
...
def print_failed():
    ...
    print("These repos failed to merge, fix manually: ")
    for repo in REPOS_ERROR:
        print(repo)
