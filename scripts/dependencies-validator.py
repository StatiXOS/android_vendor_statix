...
def suggest_edits(dependency):
    ...
    if 'remote' in dependency:
        if dependency['remote'] == 'statix':
            return
        elif dependency['remote'] == 'github' and dependency['repository'].startswith("StatiXOS"):
            print("For dependency {}, consider setting remote as 'statix'".format(dependency['repository']))
