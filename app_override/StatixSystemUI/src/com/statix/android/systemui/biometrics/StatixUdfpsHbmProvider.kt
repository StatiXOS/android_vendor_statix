...
class StatixUdfpsHbmProvider @Inject constructor(
    ...
    private var displayHal = ServiceManager.getService(PIXEL_DISPLAY_HAL).let { binder ->
    ...
    override fun enableHbm(hbmType: Int, surface: Surface?, onHbmEnabled: Runnable?) {
        ...
        if (hbmType != HBM_TYPE || displayHal.getLhbmState()) {
            onHbmEnabled?.run()
            return
        }
    ...
    override fun disableHbm(onHbmDisabled: Runnable?) {
        ...
        if (displayHal != null && !displayHal.getLhbmState()) {
            return
        }
