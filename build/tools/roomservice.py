...
if len(sys.argv) > 2:
    DEPENDENCIES_ONLY = sys.argv[2]
else:
    DEPENDENCIES_ONLY = None
    ...
    if os.path.exists(dependencies_path):
        with open(dependencies_path, "r") as dependencies_file:
            ...
            for dependency in dependencies:
                if is_in_manifest(f"{dependency['repository']}", f"{dependency['branch']}", ):
...
def main():
    ...
    if DEPENDENCIES_ONLY is not None:
        print(
    ...
    if DEPENDENCIES_ONLY is None:
        for repository in get_repositories():
            ...
            if repository_name.startswith("android_device_") and repository_name.endswith(
                ...
                subprocess.run(
                    ["repo", "sync", "--force-sync", repository_path], check=False,
                )
        ...
        print(
            "Repository for {} not found in the {} Github repository list. "
            "If this is in error, you may need to manually add it to "
            "{}{}".format(
                ORGANIZATION_NAME, DEVICE, LOCAL_MANIFESTS_PATH, LOCAL_MANIFESTS_FILE_NAME
            )
        )
