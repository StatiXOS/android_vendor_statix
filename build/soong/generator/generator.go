...
func init() {
	android.RegisterModuleType("statix_generator", GeneratorFactory)
...
func (g *Module) GenerateAndroidBuildActions(ctx android.ModuleContext) {
	...
	if depRoot == "" {
		...
		depRoot = statixExpandVariables(ctx, depRoot)
	}

	// Glob dep_files property
	for _, dep_file := range g.properties.Dep_files {
		dep_file = statixExpandVariables(ctx, dep_file)
	...
	cmd := statixExpandVariables(ctx, String(g.properties.Cmd))
